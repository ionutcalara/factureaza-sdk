...
trait Clients
{
	...
	public function updateClient(array|UpdateClient $client): Client
	{
		$request = is_array($client) ? UpdateClient::fromArray($client) : $client;

		$response = $this->mutate($request);

		return new Client(
			$this->remap($response->json('data')['updateClient'], Client::class)
		);
	}
}
