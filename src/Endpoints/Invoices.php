...
use Konekt\Factureaza\Requests\GetInvoicesByClientID;
...
trait Invoices
{
     ...
     */
    public function invoicesByClientId(string $clientId, string $orderBy = '{"documentDate": "desc","createdAt":"desc"}', $limit = 20, $offset = 0): array
    {
        $response = $this->query(new GetInvoicesByClientID($clientId, $orderBy, $limit, $offset));
        $invoices = $response->json('data')['invoices'] ?? [];

        return array_map(function ($invoice) {
            return $this->rawApiDataToInvoice($invoice);
        }, $invoices);
    }
